...
static uint16_t blend565(uint16_t a, uint16_t b, uint8_t t) {
  uint8_t ar = ((a >> 11) & 31) << 3;
  uint8_t ag = ((a >> 5) & 63) << 2;
  uint8_t ab = (a & 31) << 3;
  uint8_t br = ((b >> 11) & 31) << 3;
  uint8_t bg = ((b >> 5) & 63) << 2;
  uint8_t bb = (b & 31) << 3;
  return rgb565((ar * (255 - t) + br * t) / 255,
                (ag * (255 - t) + bg * t) / 255,
                (ab * (255 - t) + bb * t) / 255);
}
...
static float dz(float v, float deadzone) {
  if (fabsf(v) <= deadzone) return 0.0f;
  float s = v >= 0.0f ? 1.0f : -1.0f;
  return s * clampf2((fabsf(v) - deadzone) / (1.0f - deadzone), 0.0f, 1.0f);
}

static bool debounced(int pin, bool *rawPrev, bool *stable, uint32_t *changedAt) {
  ...
  if (now - *changedAt >= 30 && raw != *stable) *stable = raw;
  return *stable;
}

static void allocFB() {
  if (fb) return;
  fb = (uint16_t*)heap_caps_malloc(WL_W * WL_H * sizeof(uint16_t), MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
  if (!fb) fb = (uint16_t*)heap_caps_malloc(WL_W * WL_H * sizeof(uint16_t), MALLOC_CAP_8BIT);
}

static void clearFB(uint16_t c) {
  if (!fb) return;
  for (int i = 0; i < WL_W * WL_H; ++i) fb[i] = c;
}

static void px(int x, int y, uint16_t c) {
  if (!fb) return;
  if ((unsigned)x >= WL_W || (unsigned)y >= WL_H) return;
  fb[y * WL_W + x] = c;
}

static void rectFB(int x, int y, int w, int h, uint16_t c) {
  if (!fb) return;
  int x0 = max(0, x), y0 = max(0, y);
  int x1 = min(WL_W, x + w), y1 = min(WL_H, y + h);
  for (int yy = y0; yy < y1; ++yy) {
    uint16_t *row = fb + yy * WL_W;
    for (int xx = x0; xx < x1; ++xx) row[xx] = c;
  }
}

static void circleFB(int cx, int cy, int r, uint16_t c) {
  int r2 = r * r;
  for (int y = -r; y <= r; ++y) {
    for (int x = -r; x <= r; ++x) if (x * x + y * y <= r2) px(cx + x, cy + y, c);
  }
}

static void pushFB() {
  if (fb) tft.drawRGBBitmap(0, 0, fb, WL_W, WL_H);
}

// ---------- LiquidOS feedback ----------
static const uint8_t COLOR_COUNT = 7;
static uint8_t colorIndex = 5;
static const uint8_t waterRGB[COLOR_COUNT][3] = {
  {255,60,60},{255,145,40},{255,220,40},{60,220,90},{40,220,235},{55,125,255},{180,85,255}
};
static const uint8_t edgeRGB[COLOR_COUNT][3] = {
  {255,180,180},{255,215,155},{255,245,170},{180,255,195},{170,255,255},{185,220,255},{230,190,255}
};
...
static void motorWrite(bool on) {
  digitalWrite(WL_MOTOR_PIN, WL_MOTOR_ACTIVE_HIGH ? (on ? HIGH : LOW) : (on ? LOW : HIGH));
}
...
static void ledAll(uint8_t r, uint8_t g, uint8_t b) {
  for (int i = 0; i < WL_RGB_COUNT; ++i) leds.setPixelColor(i, leds.Color(r, g, b));
  leds.show();
}
...
// ---------- MPU6050 ----------
static bool mpuTried = false, mpuOk = false;
static uint8_t mpuAddr = 0x68;
static float ax = 0, ay = 0, az = 1, gx = 0, gy = 0, gz = 0;
static float biasAX = 0, biasAY = 0, lastAX = 0, lastAY = 0, lastAZ = 1;
...
static void initMPU() {
  if (mpuTried) return;
  mpuTried = true;
  Wire.setPins(WL_I2C_SDA, WL_I2C_SCL);
  Wire.begin(); delay(60);
  if (i2cCheck(0x68)) { mpuAddr = 0x68; mpuOk = true; }
  else if (i2cCheck(0x69)) { mpuAddr = 0x69; mpuOk = true; }
  else return;
  ...
  for (int i = 0; i < 80; ++i) { if (readMPU()) { sx += ax; sy += ay; n++; } delay(4); }
  if (n) { biasAX = sx / n; biasAY = sy / n; }
...
static void constrainP(int i) {
  float b = -0.20f;
  if (wx[i] < R) { wx[i] = R; wvx[i] *= b; } if (wx[i] > WL_W - 1 - R) { wx[i] = WL_W - 1 - R; wvx[i] *= b; }
...
static void solvePairs() {
  ...
  for (int i = 0; i < nWater; ++i) for (int j = i + 1; j < nWater; ++j) {
    ...
    wx[i] -= nx*p; wy[i] -= ny*p; wx[j] += nx*p; wy[j] += ny*p;
  }
}
...
static void updateDrops() {
  for(int i=0;i<DROP_MAX;++i) if(dropOn[i]){
    dvx[i]+=forceX*0.012f; dvy[i]+=0.072f+forceY*0.012f; dvx[i]*=0.994f; dvy[i]*=0.994f; dxp[i]+=dvx[i]; dyp[i]+=dvy[i]; dlife[i]--;
    if(dxp[i]<2){dxp[i]=2;dvx[i]*=-0.24f;} if(dxp[i]>WL_W-3){dxp[i]=WL_W-3;dvx[i]*=-0.24f;} if(dyp[i]<2){dyp[i]=2;dvy[i]*=-0.20f;}
    if(dyp[i]>WL_H || dlife[i]<=0) dropOn[i]=false;
  }
}
