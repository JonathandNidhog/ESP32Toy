...
static void solvePairs() { float td = R * 1.38f, td2 = td * td; for (int i = 0; i < nWater; ++i) for (int j = i + 1; j < nWater; ++j) { float x = wx[j] - wx[i], y = wy[j] - wy[i], d2 = x*x + y*y; if (d2 < 0.0001f || d2 >= td2) continue; float d = sqrtf(d2), p = (td - d) * 0.46f, nx = x / d, ny = y / d; wx[i] -= nx*p; wy[i] -= ny*p; wx[j] += nx*p; wy[j] += ny*p; } }
static void viscosity() { float rsq = R * R * 7.2f; for (int i = 0; i < nWater; ++i) for (int j = i + 1; j < nWater; ++j) { float x = wx[j]-wx[i], y = wy[j]-wy[i]; if (x*x+y*y >= rsq) continue; float vx = wvx[j]-wvx[i], vy = wvy[j]-wvy[i]; wvx[i] += vx*0.026f; wvy[i] += vy*0.026f; wvx[j] -= vx*0.026f; wvy[j] -= vy*0.026f; } }
...
static void stepWater(bool manual) { updateAmount(); float jx=dz(clampf2((analogRead(WL_LEFT_JOY_X_PIN)-2048)/1800.0f,-1,1),0.14f), jy=dz(clampf2((analogRead(WL_LEFT_JOY_Y_PIN)-2048)/1800.0f,-1,1),0.14f); joyFX=joyFX*0.8f+jx*0.2f; joyFY=joyFY*0.8f+jy*0.2f; bool ok=readMPU(); float tx=0,ty=0,gsx=0,gsy=0,ss=0; if(ok){float bx=ax-biasAX, by=-(ay-biasAY); if(fabsf(bx)<0.03f)bx=0; if(fabsf(by)<0.03f)by=0; mapToScreen(clampf2(bx*2.1f,-1,1),clampf2(by*2.1f,-1,1),&tx,&ty); mapToScreen(clampf2(gx*0.008f,-1.3f,1.3f),clampf2(gy*0.008f,-1.3f,1.3f),&gsx,&gsy); float jerk=fabsf(ax-lastAX)+fabsf(ay-lastAY)+fabsf(az-lastAZ); lastAX=ax;lastAY=ay;lastAZ=az; ss=jerk*6.5f+fabsf(gx)*0.0028f+fabsf(gy)*0.0028f+fabsf(gz)*0.0018f;} forceX=forceX*0.84f+clampf2(tx+joyFX*0.75f,-1.35f,1.35f)*0.16f; forceY=forceY*0.84f+clampf2(ty+joyFY*0.75f,-1.35f,1.35f)*0.16f; for(int i=0;i<nWater;++i){wvx[i]+=forceX*0.27f+gsx*0.18f; wvy[i]+=forceY*0.27f+gsy*0.18f+0.012f; wvx[i]*=0.986f; wvy[i]*=0.986f; wvx[i]=clampf2(wvx[i],-4.2f,4.2f); wvy[i]=clampf2(wvy[i],-4.2f,4.2f); wx[i]+=wvx[i]; wy[i]+=wvy[i]; constrainP(i);} solvePairs(); solvePairs(); solvePairs(); for(int i=0;i<nWater;++i)constrainP(i); viscosity(); updateDrops(); if(splashCooldown>0)splashCooldown--; if(ss>1.25f&&splashCooldown<=0){splash(forceX+gsx*0.35f,forceY+gsy*0.35f,ss);splashCooldown=9;} if(manual&&splashCooldown<=0){splash(forceX,forceY,2.0f);splashCooldown=8;} }
