...
// ============================================================
// ESP32Toy Liquid OS - No Audio Next Build
// Board: ESP32-S3 N16R8
// Display: 1.8 inch ST7735
// Includes:
//   Water app
//   Doom placeholder app
//   Settings screen rotation
//   A key IO15 confirm / action
//   B key IO14 back / exit
//   RGB LED board IO47, 4 pixels, 3V3 powered
// Audio module is intentionally skipped in this build.
// ============================================================
...
Adafruit_NeoPixel leds(RGB_LED_COUNT, RGB_LED_PIN, NEO_GRB + NEO_KHZ800);
...
void mapBaseVectorToScreen(float baseX, float baseY, float *outX, float *outY) {
  switch (displayRotation & 3) {
    case 0: *outX = baseX;  *outY = baseY;  break;
    case 1: *outX = baseY;  *outY = -baseX; break;
    case 2: *outX = -baseX; *outY = -baseY; break;
    default: *outX = -baseY; *outY = baseX; break;
  }
}
...
const uint16_t C_DOOM_RED = rgb565(220, 35, 25);
...
uint16_t waterEdgeColor() { return waterEdgeColors[waterColorIndex]; }
...
void ledRainbowStep(uint8_t offset, uint8_t div) {
  for (int i = 0; i < RGB_LED_COUNT; i++) {
    uint32_t c = ledWheel(offset + i * 64);
    uint8_t r = (uint8_t)(c >> 16);
    uint8_t g = (uint8_t)(c >> 8);
    uint8_t b = (uint8_t)c;
    leds.setPixelColor(i, leds.Color(r / div, g / div, b / div));
  }
  leds.show();
}
...
void ledWaterColor() {
  switch (waterColorIndex) {
    case 0: ledSetAll(44, 0, 0); break;
    case 1: ledSetAll(44, 14, 0); break;
    case 2: ledSetAll(38, 34, 0); break;
    case 3: ledSetAll(0, 38, 8); break;
    case 4: ledSetAll(0, 30, 30); break;
    case 5: ledSetAll(0, 8, 44); break;
    default: ledSetAll(28, 0, 38); break;
  }
}

void ledMenu() { ledSetAll(8, 12, 24); }
void ledSettings() { ledSetAll(12, 16, 24); }
void ledDoomBase() { ledSetAll(35, 0, 0); }
void ledOff() { ledSetAll(0, 0, 0); }
...
bool keyARawPrev = HIGH;
bool keyAStable = HIGH;
bool keyAPressedEdge = false;
uint32_t keyAChangedAt = 0;

bool keyBRawPrev = HIGH;
bool keyBStable = HIGH;
bool keyBPressedEdge = false;
uint32_t keyBChangedAt = 0;
...
void motorWrite(bool on) {
  if (MOTOR_ACTIVE_HIGH) digitalWrite(MOTOR_PIN, on ? HIGH : LOW);
  else digitalWrite(MOTOR_PIN, on ? LOW : HIGH);
}
...
float biasAX = 0.0f, biasAY = 0.0f;
float lastAX = 0.0f, lastAY = 0.0f, lastAZ = 1.0f;
float forceX = 0.0f, forceY = 0.0f;
float joystickForceX = 0.0f, joystickForceY = 0.0f;
...
void calibrateMPU() {
  ...
  int validSamples = 0;
  for (int i = 0; i < 120; i++) {
    if (readMPU()) { sumAX += ax; sumAY += ay; validSamples++; }
    delay(6);
  }
  if (validSamples > 0) { biasAX = sumAX / validSamples; biasAY = sumAY / validSamples; }
...
float dropX[DROP_COUNT_MAX], dropY[DROP_COUNT_MAX], dropVX[DROP_COUNT_MAX], dropVY[DROP_COUNT_MAX], dropRadius[DROP_COUNT_MAX];
...
void constrainWaterParticle(int i) {
  ...
  if (wx[i] > screenW - 1 - WATER_RADIUS) { wx[i] = screenW - 1 - WATER_RADIUS; wvx[i] *= bounce; }
  if (wy[i] < WATER_RADIUS) { wy[i] = WATER_RADIUS; wvy[i] *= bounce; }
  if (wy[i] > screenH - 1 - WATER_RADIUS) { wy[i] = screenH - 1 - WATER_RADIUS; wvy[i] *= bounce; }
}
...
void emitSplash(float splashFX, float splashFY, float strength) {
  ...
  if (len > 0.05f) { upX = -splashFX / len; upY = -splashFY / len; }
  ...
  for (int a = 0; a < surfacePickCount; a++) {
    int best = a;
    for (int b = a + 1; b < activeWaterCount; b++) if (surfaceScores[b] > surfaceScores[best]) best = b;
    float tempScore = surfaceScores[a]; surfaceScores[a] = surfaceScores[best]; surfaceScores[best] = tempScore;
    int tempId = surfaceIds[a]; surfaceIds[a] = surfaceIds[best]; surfaceIds[best] = tempId;
  }
  ...
  for (int n = 0; n < count; n++) {
    ...
    spawnDrop(wx[id], wy[id], wvx[id] + upX * launch + sideX * side * spread, wvy[id] + upY * launch + sideY * side * spread, random(1, 3));
    wvx[id] += upX * 0.14f;
    wvy[id] += upY * 0.14f;
  }
}
...
void updateDrops() {
  for (int i = 0; i < DROP_COUNT_MAX; i++) {
    ...
    if (dropX[i] > screenW - 3) { dropX[i] = screenW - 3; dropVX[i] *= -0.24f; }
    ...
    if (dropY[i] > screenH || dropLife[i] <= 0) dropActive[i] = false;
  }
}

void updateJoystickWaterForce() {
  float joyMappedX = 0.0f, joyMappedY = 0.0f;
...
void updateWater() {
  ...
  mapBaseVectorToScreen(clampf(baseTiltX * 2.10f, -1.0f, 1.0f), clampf(baseTiltY * 2.10f, -1.0f, 1.0f), &tiltX, &tiltY);
  mapBaseVectorToScreen(clampf(gx * 0.008f, -1.3f, 1.3f), clampf(gy * 0.008f, -1.3f, 1.3f), &gyroScreenX, &gyroScreenY);
  ...
  for (int i = 0; i < activeWaterCount; i++) constrainWaterParticle(i);
  applyWaterViscosity();
...
void drawWaterBody() {
  ...
  for (int gyIndex = 0; gyIndex < fieldH; gyIndex++) {
    ...
    for (int gxIndex = 0; gxIndex < fieldW; gxIndex++) {
      ...
      if ((!inside || last) && runStart >= 0) {
        ...
        CV.fillRect(runStart * GRID_STEP, gyIndex * GRID_STEP, (runEnd - runStart + 1) * GRID_STEP, GRID_STEP, waterColor());
        runStart = -1;
      }
    }
  }
  ...
  for (int gyIndex = 0; gyIndex < fieldH; gyIndex++) {
    for (int gxIndex = 0; gxIndex < fieldW; gxIndex++) {
      ...
      if (topEdge) CV.fillRect(gxIndex * GRID_STEP, gyIndex * GRID_STEP, GRID_STEP, 1, waterEdgeColor());
    }
  }
}
...
void drawDrops() {
  for (int i = 0; i < DROP_COUNT_MAX; i++) {
    ...
    if (dropRadius[i] >= 2) CV.drawPixel((int)dropX[i] - 1, (int)dropY[i] - 1, C_WHITE);
  }
}
...
const char *menuSubtitles[MENU_COUNT] = {"Interactive liquid toy", "Placeholder app", "Display rotation"};
int menuIndex = 0;
...
void drawMenu() {
  ...
  for (int i = 0; i < MENU_COUNT; i++) {
    ...
    uint16_t accent = (i == 1) ? C_DOOM_RED : waterColor();
    drawPanel(boxX, y, boxW, boxH, selected ? C_PANEL_SELECTED : C_PANEL, selected ? accent : C_LINE);
    CV.setTextSize(1);
    CV.setTextColor(selected ? C_WHITE : C_MUTED);
    CV.setCursor(boxX + 10, y + boxH / 2 - 3);
    CV.print(menuItems[i]);
    if (selected) CV.fillCircle(boxX + boxW - 12, y + boxH / 2, 3, accent);
  }
...
void updateMenu() {
  ...
  float joyScreenX = 0.0f, joyScreenY = 0.0f;
...
void drawDoom() {
  CV.fillScreen(rgb565(10, 4, 4));
  drawCenteredText("DOOM", 12, 3, C_DOOM_RED);
  drawCenteredText("APP STUB", 42, 1, C_WHITE);

  int cx = screenW / 2;
  int cy = screenH / 2 + 4;
  CV.drawRoundRect(cx - 52, cy - 22, 104, 44, 5, C_DOOM_RED);
  CV.setTextSize(1);
  CV.setTextColor(C_MUTED);
  CV.setCursor(cx - 44, cy - 13);
  CV.print("Engine later");
  CV.setCursor(cx - 44, cy + 1);
  CV.print("A: Fire test");
  CV.setCursor(cx - 44, cy + 15);
  CV.print("B: Back");
  ...
  CV.setTextColor(C_MUTED);
  CV.print("J ");
  CV.print(joyScreenX, 1);
  CV.print(" ");
  CV.print(joyScreenY, 1);

  pushCanvas();
}

void updateDoom() {
  static uint32_t lastDoomLedMs = 0;
  static uint8_t doomLedPhase = 0;

  if (millis() - lastDoomLedMs > 80) {
    doomLedPhase += 8;
    for (int i = 0; i < RGB_LED_COUNT; i++) {
      uint8_t v = (doomLedPhase + i * 50) & 127;
      if (v > 63) v = 127 - v;
      leds.setPixelColor(i, leds.Color(24 + v / 3, 0, 0));
    }
    leds.show();
    lastDoomLedMs = millis();
  }
  ...
  if (keyBPressedEdge || (joyPressed && !joyPressedPrev)) {
    mode = MODE_MENU;
    ledMenu();
    motorPulse(120);
  }
...
void drawSettings() {
  ...
  drawCenteredText("A: rotate  B: back", screenH - 12, 1, C_MUTED);
  pushCanvas();
}
...
void drawBootFrame(float t) {
  ...
  if (t > 0.73f) drawCenteredText("ESP32 TOY", screenH - 24, 1, C_WHITE);
  pushCanvas();
}
...
void runBootAnimation() {
  ...
  while (millis() - startMs < 1800) {
    float t = (millis() - startMs) / 1800.0f;
    drawBootFrame(t);
    ledRainbowStep((uint8_t)(t * 255.0f), 5);
    delay(20);
  }
  drawBootFrame(1.0f);
  delay(140);
...
void updateInputs() {
  ...
  bool rawA = digitalRead(KEY_A_PIN);
  keyAPressedEdge = false;
  if (rawA != keyARawPrev) { keyARawPrev = rawA; keyAChangedAt = now; }
  if (now - keyAChangedAt >= KEY_DEBOUNCE_MS && rawA != keyAStable) {
    keyAStable = rawA;
    if (keyAStable == LOW) keyAPressedEdge = true;
  }

  bool rawB = digitalRead(KEY_B_PIN);
  keyBPressedEdge = false;
  if (rawB != keyBRawPrev) { keyBRawPrev = rawB; keyBChangedAt = now; }
  if (now - keyBChangedAt >= KEY_DEBOUNCE_MS && rawB != keyBStable) {
    keyBStable = rawB;
    if (keyBStable == LOW) keyBPressedEdge = true;
  }
}
...
void handleWaterControls() {
  if (keyAPressedEdge) nextWaterColor();
